...
window.ARTICLE_CONTENT = {
  ...
  'start-norwegian': {
    ...
    sections: [
      ...
      },
      {
        ...
        body: '<p>Стандартный путь — подача из страны, где вы проживаете, через визовый центр VFS. Подача из Норвегии возможна не всегда — нужны определённые основания.</p><p>Подробно про оба варианта — в статье <a data-article="where-to-apply">«Где подавать заявление: из России или из Норвегии»</a>.</p>'
      },
      {
  ...
  'start-eu': {
    ...
    sections: [
      ...
      {
        title: 'Где подавать: из России или из Норвегии',
        body: '<p>По кейсам сообщества самый частый сценарий — подача в полиции в Норвегии. Для въезда до подачи может пригодиться гостевая виза C по директиве 2004/38/EC через консульство третьей страны Шенгена. Подробно — в статье <a data-article="visa-c-guest">«Гостевая виза C: как приехать к супругу»</a> (путь 3).</p>'
      }
    ],
  ...
  'start-skilled': {
    ...
    sections: [
      ...
      },
      {
        ...
        body: '<p>Стандартный путь — подача из страны проживания через визовый центр VFS. Если вы сами подходите под критерии skilled worker по образованию, может быть возможна подача из Норвегии. Подробно — в статье <a data-article="where-to-apply">«Где подавать заявление: из России или из Норвегии»</a>.</p><p>Во время ожидания работать на норвежского или европейского работодателя нельзя, даже удалённо.</p>'
      },
      {
        title: 'К чему морально готовиться',
  ...
  'student-spouse': {
    ...
    actionNow: [
      ...
      'Если доход на грани — посмотрите статью <a href="#"><a data-article="income-req">«Требование по доходу принимающей стороны»</a></a> на предмет исключений.'
    ],
    sections: [
      {
  ...
  'refugee-6-month-window': {
    ...
    faq: [
      { q: 'Что если 6 месяцев уже прошли?', a: 'Можно подавать как обычная семейная миграция, но тогда применяется требование по доходу. Подробнее в статье <a href="#"><a data-article="income-req">«Требование по доходу принимающей стороны»</a></a>.' },
  ...
  'age-24-rule': {
    ...
    faq: [
      ...
      { q: 'Как доказать 2 года совместной жизни до переезда супруга?', a: 'Документы о совместной аренде, общие счета, фото и переписка за весь период. Подробнее в статье <a href="#"><a data-article="samboer-cohabitation">«Сожительство (samboer): как подтвердить 2 года совместной жизни»</a></a>.' },
  ...
  'application-documents': {
    ...
    faq: [
      ...
      { q: 'Нужен ли мой диплом при подаче?', a: 'Для воссоединения с супругом диплом заявителя НЕ требуется. Он нужен только если подаётесь из Норвегии как Skilled worker (через собственное образование). Подробнее — в статье <a href="#"><a data-article="where-to-apply">«Где подавать заявление: из России или из Норвегии»</a></a>.' },
